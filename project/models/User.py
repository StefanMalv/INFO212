...
def order_car(car_id, customer_id):
    car = _get_connection().execute_query("MATCH (c:Car {id: $car_id}) RETURN c;", car_id=car_id)
    customer = _get_connection().execute_query("MATCH (c:Customer {id: $customer_id}) RETURN c;", customer_id=customer_id)
    
    # Check if the customer has already booked another car
    existing_order = _get_connection().execute_query("MATCH (customer:Customer {id: $customer_id})-[:ORDERED]->(c:Car) RETURN c;", customer_id=customer_id)
    
    if existing_order:
        return None, "Customer has already booked another car"
    
    if car['status'] == 'available':
        order = _get_connection().execute_query("MATCH (c:Car {id: $car_id}) ""SET c.status = 'booked' ""MERGE (customer:Customer {id: $customer_id})-[:ORDERED]->(c) ""RETURN c;", car_id=car_id, customer_id=customer_id)
...
def cancel_order_car(car_id, customer_id):
    ...
    existing_order = _get_connection().execute_query("MATCH (customer:Customer {id: $customer_id})-[:ORDERED]->(c:Car {id: $car_id}) RETURN c;", customer_id=customer_id, car_id=car_id)
    
    if existing_order:
        _get_connection().execute_query("MATCH (customer:Customer {id: $customer_id})-[r:ORDERED]->(c:Car {id: $car_id}) " "SET c.status = 'available' ""DELETE r " "RETURN c;", customer_id=customer_id, car_id=car_id)
...
def rent_car(car_id, customer_id):
    ...
    if existing_order:
        _get_connection().execute_query(
            "MATCH (customer:Customer {id: $customer_id})-[r:ORDERED]->(c:Car {id: $car_id}) ""SET c.status = 'rented' ""DELETE r ""MERGE (customer)-[:RENTED]->(c) ""RETURN c;", customer_id=customer_id, car_id=car_id)
...
def return_car(car_id, customer_id, car_status):
    ...
    existing_rental = _get_connection().execute_query("MATCH (customer:Customer {id: $customer_id})-[:RENTED]->(c:Car {id: $car_id}) RETURN c;", customer_id=customer_id, car_id=car_id)
    
    if existing_rental:
        _get_connection().execute_query("MATCH (customer:Customer {id: $customer_id})-[r:RENTED]->(c:Car {id: $car_id}) ""SET c.status = $car_status ""DELETE r ""RETURN c;", customer_id=customer_id, car_id=car_id, car_status=car_status)
...
def create_employee(name, adress, branch):
    ...
    employee = _get_connection().execute_query("MERGE (a:Employee {name: $name, adress: $adress, branch: $branch, id: $id}) RETURN a;", name=new_employee.name, adress=new_employee.adress, branch=new_employee.branch, id=new_employee.id)
...
def update_employee(employee_id, name, adress, branch):
    employee = _get_connection().execute_query("MATCH (a:Employee {id: $id}) SET a.name = $name, a.adress = $adress, a.branch = $branch RETURN a;", id=employee_id, name=name, adress=adress, branch=branch)
...
def create_car(brand, model, year, location):
    ...
    car = _get_connection().execute_query("MERGE (a:Car {year: $year, location: $location, model: $model, id: $id, brand: $brand, status: $status}) RETURN a;", year=new_car.year, location=new_car.location, model=new_car.model, id=new_car.id, brand=new_car.brand, status=new_car.status)
...
def update_car(id, brand, model, year, location, status):
    car = _get_connection().execute_query("MATCH (a:Car {id: $id}) SET a.brand = $brand, a.model = $model, a.year = $year, a.location = $location , a.status = $status RETURN a;", id=id, brand=brand, model=model, year=year, location=location, status=status)
...
def create_customer(name, age, adress):
    ...
    customer = _get_connection().execute_query("MERGE (c:CUSTOMER {adress: $adress, name: $name, id: $id, age: $age}) RETURN c;", adress=new_customer.adress, name=new_customer.name, age=new_customer.age)
...
def update_customer(id, name, age, adress):
    customer = _get_connection().execute_query("MATCH (c:Customer {id: $id}) SET c.name = $name, c.age = $age, c.adress = $adress RETURN c;", id=id, name=name, age=age, adress=adress)
